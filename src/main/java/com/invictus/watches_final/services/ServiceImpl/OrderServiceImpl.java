...
//    @Override
//    public void confirmOrder(UUID orderId) { zato sto cemo automatski preko webhook-a poslep lacanja menjati status a vec nam je gore status pending
//    }                                         a front ce pomoci za prvi korak, sto znaci nemamo potrebe za ovom metodom jer necce i admin rucno odobravati
...
//    Cart cart = cartRepo.findByUser(order.getUser())
//            .orElseThrow(() -> new EntityNotFoundException("Cart not found"));
//
//    for (OrderItem item : order.getOrderItems()) {
//        cart.addItem(item.getWatch(), item.getAmount(), item.getPrice());
//    }
//
//    cartRepo.save(cart);
    }
    ...
    //cancelOrder() se poziva samo na PENDING order.
    //Nakon što Stripe uplata prođe, order se više ne može cancelovati
